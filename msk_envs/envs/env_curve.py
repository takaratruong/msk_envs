...
class CurvedTrackEnv(MSKEnv):
    def __init__(
        ...
        # Center of the curve circle in world space
        # Agent starts at x=0, z=0 facing +x
        # Therefore the circle center must be at z=+R
        ...
        self.fwd_axis = torch.tensor(
            build_axis(FWD_IDX, 1.0),
            device=self.device,
            dtype=torch.float32
        ).unsqueeze(0)

        self.cos_angle_threshold = torch.cos(
            torch.deg2rad(
                torch.tensor(angle_tolerance, device=self.device)
            )
        )
    ...
    def _get_curved_metrics(self):
        ...
        # Pointing from center to agent
        radial_vec = pos_2d / (dist_from_center + 1e-6)

        # TANGENT CALCULATION:
        # If radial_vec is (0, -1), we want target_dir to be (1, 0)
        # Therefore: target_dir = [-radial_vec.z, radial_vec.x]
    ...
    def _compute_raw_reward_dict(self):
        ...
        root_vel_2d = self.body_velocities[:, self.root_id, [FWD_IDX, SIDE_IDX]]
