...
def build_motion(motion_data, col_names, qpos_id_lookup: dict[str, int]):
    ...
    for i, dof in enumerate(col_names):
        ...
        if dof not in qpos_id_lookup:
            print(f"Warning in motion generation: {dof} not in model")
            continue

        qpos_id = qpos_id_lookup[dof]
        motion[:, qpos_id + 1] = motion_data[i, :]
    return motion
...
def parse_mot(
    ...
    motion_data = get_raw_data(lines, col_names)
    ...
    motion_data, col_names = rot_to_quat(motion_data, col_names, in_degrees)

    # Deg to rad
    if in_degrees:
        motion_data = joints_to_radians(motion_data)

    # Make time start at 0
    motion_data[0, :] = motion_data[0, :] - motion_data[0, 0]

    # Now we build the motion
    motion = build_motion(motion_data, col_names, load_result.qpos_id_lookup)
    return motion
