...
class ImitateEnv(MSKEnv):
    def __init__(
        ...
        ref_q[:, self.qpos_id_lookup["pelvis_ty"]] -= 0.135
    ...
    def _compute_raw_reward_dict(self):
        ...
        q_err_sum = torch.sum(q_err, dim=-1)
        u_err_sum = torch.sum(u_err, dim=-1)
        bp_err_sum = torch.sum(bp_err, dim=-1)

        rew_track_q = torch.exp(-0.1 * q_err_sum)
        rew_track_u = torch.exp(-0.01 * u_err_sum)
        rew_track_bp = torch.exp(-0.3 * bp_err_sum)
